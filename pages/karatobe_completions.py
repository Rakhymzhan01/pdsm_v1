...
dash.register_page(__name__, path='/karatobe/comp')
...
def layout():
    if not current_user.is_authenticated:
        ...
        return html.Div([
            navigation.navbar,
            dcc.Interval(id='interval_comp', interval=86400000*7, n_intervals=0),
            dbc.Col([
                html.Br(),
                dcc.Loading(children=html.Div(id='completion_datatable')),
                html.Br(),
                html.Button('Добавить', id='add_completion', n_clicks=0),
                html.Button('Сохранить', id='save_completion_table', n_clicks=0),
                html.Div(id='completion_notification', children=[]),
...
@callback(
    Output('completion_datatable', 'children'),
    Input('interval_comp', 'n_intervals')
)
def populate_perf_datatable(n_intervals):
    df_comp = pd.read_sql_table("completions", con=engine)
    return [
...
@callback(
    ...
    Input('add_completion', 'n_clicks'),
    [State('comp-table', 'data'), State('comp-table', 'columns')]
)
...
@callback(
    Output('completion_notification', 'children'),
    Input('save_completion_table', 'n_clicks'),
...
def df_to_postgres(n_clicks, dataset):
    if n_clicks > 0:
        ...
        df.to_sql("completions", con=engine, if_exists='replace', index=False)
        return [
