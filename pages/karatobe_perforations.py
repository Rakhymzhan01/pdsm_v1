...
dash.register_page(__name__, path='/karatobe/perf')
...
def layout():
    if not current_user.is_authenticated:
        ...
        return html.Div([
            navigation.navbar,
            dcc.Interval(id='interval_perf', interval=86400000*7, n_intervals=0),
            dbc.Col([
                html.Br(),
                dcc.Loading(children=html.Div(id='perforation_datatable')),
                html.Br(),
                html.Button('Добавить', id='add_perforation', n_clicks=0),
                html.Button('Сохранить', id='save_perforation_table', n_clicks=0),
                html.Div(id='perforation_notification', children=[]),
...
@callback(
    Output('perforation_datatable', 'children'),
    Input('interval_perf', 'n_intervals')
)
def populate_perf_datatable(n_intervals):
    df_perf = pd.read_sql_table("perfs", con=engine)
    return [
        dash_table.DataTable(
            id='perf-table',
            columns=[
                {"name": i, "id": i, "deletable": False, "selectable": True, "hideable": True}
                for i in df_perf.columns
            ],
...
@callback(
    ...
    Input('add_perforation', 'n_clicks'),
    [State('perf-table', 'data'), State('perf-table', 'columns')]
)
...
@callback(
    Output('perforation_notification', 'children'),
    Input('save_perforation_table', 'n_clicks'),
    State('perf-table', 'data')
)
def df_to_postgres(n_clicks, dataset):
    if n_clicks > 0:
        ...
        df.to_sql("perfs", con=engine, if_exists='replace', index=False)
        return [
